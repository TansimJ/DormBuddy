...
class _AddDormFormState extends State<AddDormForm> {
  ...
  // --- Required fields validation for each section ---
  bool _validateRequiredFields(int section) {
    switch (section) {
      case 1:
        return (widget.formData['dormitory_name']?.toString().isNotEmpty ?? false) &&
            (widget.formData['dormitory_type']?.toString().isNotEmpty ?? false) &&
            (widget.formData['gender_preference']?.toString().isNotEmpty ?? false);
      case 2:
        return (widget.formData['address_line']?.toString().isNotEmpty ?? false) &&
            (widget.formData['city']?.toString().isNotEmpty ?? false) &&
            (widget.formData['state']?.toString().isNotEmpty ?? false);
      case 6:
        return (widget.formData['monthly_rate_(rm)']?.toString().isNotEmpty ?? false) &&
            (widget.formData['deposit_required_(rm)']?.toString().isNotEmpty ?? false) &&
            (widget.formData['contract_flexibility']?.toString().isNotEmpty ?? false);
      default:
        return true;
    }
  }
  ...
  void _handleNextPressed() {
    if (!_validateRequiredFields(widget.currentSection)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Please fill all required fields before continuing.'),
          backgroundColor: Colors.red[700],
        ),
      );
      return;
    }
    widget.onSectionChange(widget.currentSection + 1);
  }
  ...
  Widget _buildNavigationButtons() {
    return Row(
      ...
      children: [
        ...
        if (widget.currentSection == 6)
          ElevatedButton(
            onPressed: _validateRequiredFields(6)
                ? () => widget.onSectionChange(7)
                : null,
            style: ElevatedButton.styleFrom(
              backgroundColor: const Color(0xFF800000),
              foregroundColor: Colors.white,
  ...
  Widget _buildSection1() {
    return Column(
      children: [
        ...
        _buildTextInput('Dormitory Name', 'dormitory_name', isRequired: true),
        const SizedBox(height: 16),
        _buildDropdown('Dormitory Type', 'dormitory_type', ['Shared', 'Studio'], isRequired: true),
        const SizedBox(height: 16),
        _buildDropdown('Gender Preference', 'gender_preference', ['Men', 'Women', 'Any'], isRequired: true),
        const SizedBox(height: 16),
  ...
  Widget _buildSection2() {
    return Column(
      children: [
        ...
        _buildTextInput('Address Line', 'address_line', isRequired: true),
        const SizedBox(height: 16),
        _buildTextInput('City', 'city', isRequired: true),
        const SizedBox(height: 16),
        _buildTextInput('State', 'state', isRequired: true),
        const SizedBox(height: 16),
        _buildTextInput('Distance to UTM KL (km)', 'distance_to_utm_kl_(km)', isRequired: false, hint: 'e.g., 2.5'),
        const SizedBox(height: 16),
  ...
  Widget _buildSection5() {
    return Column(
      children: [
        ...
        _buildTextInput('Total Number of Rooms', 'total_number_of_rooms', isRequired: false, hint: 'e.g., 10'),
        const SizedBox(height: 16),
        _buildTextInput('Available Rooms', 'available_rooms', isRequired: false, hint: 'e.g., 3'),
        const SizedBox(height: 16),
        _buildTextInput('Occupied Rooms', 'occupied_rooms', isRequired: false, hint: 'e.g., 7'),
        const SizedBox(height: 16),
  ...
  Widget _buildSection6() {
    return Column(
      children: [
        ...
        _buildTextInput('Monthly Rate (RM)', 'monthly_rate_(rm)', isRequired: true, hint: 'e.g., 500'),
        const SizedBox(height: 16),
        _buildTextInput('Deposit Required (RM)', 'deposit_required_(rm)', isRequired: true, hint: 'e.g., 1000'),
        const SizedBox(height: 8),
        ...
        const SizedBox(height: 16),
        _buildDropdown('Contract Flexibility', 'contract_flexibility', ['Monthly', '3 Months', '6 Months', '1 Year'], isRequired: true),
      ],
    );
  }
